...
if os.path.exists(".pass"):
    ...
    while True:
        ...
        if re.match("^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{6,}$",master):
...
while True:
    ...
    # either creates a custom-length password with the user's choice of character combinations, or receives a custom password
        ...
        if ch==1:
            ...
            print("You can either choose 'a' (or press enter), or otherwise type out the representative numbers")
            while True:
                choices = input("Should your password have:\n1) Lowercase characters\n2) Uppercase characters\n3) Symbols/Punctuation\n4) Digits\na) All of the above\nChoice: ")
                if 'a' in choices and choices!='a':
                    print("You can either choose a), or otherwise type out the representative numbers")
                    continue
                break
            bank=[]
            if choices=='':
                bank += string.ascii_lowercase + string.ascii_uppercase + string.digits + string.punctuation
            elif choices=='a':
                bank += string.ascii_lowercase + string.ascii_uppercase + string.digits + string.punctuation
            elif '1' in choices:
        ...
        if mail.startswith('y'):
            ...
            mail=False
        if mail and site:
            mail = input("Enter your ysername/e-mail: ")
    ...
    # lets the user edit the line(s) the user wishes
        ...
        num=1
        show()
        x = input("What line(s) do you want to edit (e.g. Line: 3,1 or Line: 4)?\nLine: ")
    ...
    # lets the user change the masterpassword if it matches certain criteria, and prints relevant messages, letting him know what is wrong with the one he entered
        ...
        while True:
            ...
            if pw!=master and re.match("^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{6,}$",pw):
                f=open(".pass",'r')
                lines = f.read().splitlines()
                f.close()
                f=open(".pass",'w')
